...
function openRsvpModal() {
  openModal("rsvpModal");
  // Pre-fill nama dari URL param
  const nameInput = document.getElementById("rsvp-name");
  if (!nameInput.value && DATA.guest && DATA.guest !== "Nama Tamu") {
    nameInput.value = DATA.guest;
  }
  // Tampilkan hint grup dari nama tamu URL
  const hint = document.getElementById("grup-hint");
  if (hint && DATA.guest) hint.textContent = "";
}

function switchRsvpTab(tab) {
  const isForm = tab === "form";
  document.getElementById("rsvp-tab-form").style.display = isForm
    ? "block"
    : "none";
  document.getElementById("rsvp-tab-list").style.display = isForm
    ? "none"
    : "block";
  document.getElementById("tab-form").classList.toggle("active", isForm);
  document.getElementById("tab-list").classList.toggle("active", !isForm);

  if (!isForm && !ucapanLoaded) {
    fetchUcapan();
  }
}
...
async function fetchUcapan() {
  ...
  document.getElementById("ucapan-loading").style.display = "block";
  ...
  try {
    ...
    const url = DATA.rsvpWebhook + "?action=list";
    const res = await fetch(url, { method: "GET" });
    const json = await res.json();
    ucapanData = json.data || [];
    ucapanLoaded = true;
    renderUcapanList();
  } catch (err) {
    console.error("[RSVP] fetchUcapan gagal:", err);
    showUcapanEmpty();
  }
}

function renderUcapanList() {
  document.getElementById("ucapan-loading").style.display = "none";

  // Update badge
  const badge = document.getElementById("ucapan-badge");
  if (ucapanData.length > 0) {
    badge.textContent = ucapanData.length > 99 ? "99+" : ucapanData.length;
  ...
  listEl.innerHTML = `<div class="ucapan-list-wrap">${ucapanData
    .map(renderUcapanCard)
    .join("")}</div>`;
}
...
function renderUcapanCard(item) {
  ...
  const timeDisplay = parseAndFormatTimestamp(item.timestamp);
  const badgeClass =
    item.attendance === "hadir" ? "ucapan-badge-hadir" : "ucapan-badge-tidak";
  const badgeLabel = item.attendance === "hadir" ? "Hadir" : "Tidak Hadir";
  const commentHtml = item.comment
    ? `<div class="ucapan-text">${escapeHtml(item.comment)}</div>`
    : "";
    ...
    <div class="ucapan-card">
      ...
      <div class="ucapan-body">
        <div class="ucapan-header">
          <span class="ucapan-name">${escapeHtml(item.name)}</span>
          <span class="ucapan-badge-status ${badgeClass}">${badgeLabel}</span>
        </div>
        ${commentHtml}
        <div class="ucapan-time">${timeDisplay}</div>
      </div>
    </div>
  `;
}

function showUcapanEmpty() {
  document.getElementById("ucapan-loading").style.display = "none";
  document.getElementById("ucapan-empty").style.display = "block";
}
...
async function submitRsvp() {
  ...
  const grup = document.getElementById("rsvp-grup").value.trim();
  const rawPhone = document.getElementById("rsvp-phone").value.trim();
  const attendance = document.getElementById("rsvp-attendance-select").value;
  ...
  // Normalisasi nomor: strip leading 0, gabung dengan +62
  const phone = rawPhone ? "+62" + rawPhone.replace(/^0+/, "") : "";
  ...
  if (DATA.rsvpWebhook) {
    try {
      await fetch(DATA.rsvpWebhook, {
        method: "POST",
        mode: "no-cors", // wajib untuk Apps Script doPost
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    } catch (err) {
      console.error("[RSVP] Gagal kirim:", err);
    }
  } else {
    console.warn("[RSVP] rsvpWebhook belum diisi di data.json");
  }

  // Optimistic update: sisipkan ke cache lalu langsung render ulang list
  ucapanData.unshift({
    name,
    grup,
    attendance,
  ...
  ucapanLoaded = true;
  renderUcapanList(); // ← render sekarang, sebelum pindah ke layar sukses
...
function resetRsvpForm() {
  ...
  document.getElementById("rsvp-grup").value = "";
  document.getElementById("rsvp-phone").value = "";
  document.getElementById("rsvp-comment").value = "";
  document.getElementById("rsvp-attendance-select").value = "";
  rsvpAttendance = "";
}
...
// Parse timestamp dari berbagai format (ISO, sheet string, dsb) lalu format ulang
function parseAndFormatTimestamp(raw) {
  if (!raw) return "";
  // Jika sudah dalam format dd-mm-yyyy hh:mm WIB, kembalikan langsung
  if (/^\d{2}-\d{2}-\d{4}/.test(raw)) return raw;
  // Coba parse sebagai Date (ISO atau format lain)
  const date = new Date(raw);
  if (isNaN(date.getTime())) return raw; // tidak bisa di-parse, tampilkan apa adanya
...
async function loadData() {
  try {
    ...
    const DATA = {
      ...
      music:
        "https://www.image2url.com/r2/default/audio/1782461975669-2db6e4c8-22a0-4615-9fdf-11792611a97a.mp3",
      greeting: {
        opening:
